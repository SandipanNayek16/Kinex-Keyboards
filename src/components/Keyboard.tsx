...
export const Keyboard = forwardRef<KeyboardRefs, KeyboardProps>(
  ({ keycapMaterial, knobColor, ...props }, ref) => {
    ...
    // Configure textures (these are stable operations, textures are cached by useTexture)
    ...
    // Memoize all materials to prevent per-render allocation
    const defaultKeycapMat = useMemo(
      () =>
    ...
    const knobMat = useMemo(
      () =>
        new THREE.MeshStandardMaterial({
          color: knobColor || "#2A2A2E",
          roughness: 0.35,
          metalness: 0.9,
          bumpMap: knurlTexture,
          bumpScale: 0.8,
        }),
      [knobColor, knurlTexture],
    );
    ...
    const switchContactsMat = useMemo(
      () =>
        new THREE.MeshStandardMaterial({
          color: "#FFCF5F",
          roughness: 0.1,
